...
fn david(input: &[u8]) -> Option<usize> {
    let mut idx = 0;
    ...
    while let Some(slice) = input.get(idx..idx + 4) {
        ...
        } else if state.count_ones() == 4 as u32 {
...
fn main() {
    ...
    let n = david(INPUT.as_bytes()).unwrap();
    println!("start of packet occurs at index {n}");
}
